...
def show_all_pokemons(request):
    ...
    now = localtime()
    for pokemon_entity in PokemonEntity.objects.filter(appeared_at__lt=now, disappeared_at__gte=now):
        add_pokemon(
...
def show_pokemon(request, pokemon_id):
    ...
    folium_map = folium.Map(location=MOSCOW_CENTER, zoom_start=12)
    now = localtime()
    pokemon_entities = Pokemon.objects.get(id=pokemon_id).pokemon_entities.filter(appeared_at__lt=now, disappeared_at__gte=now)
